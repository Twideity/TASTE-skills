...
def _enrich(year:int,row:dict[str,Any])->None:
    ...
    if isinstance(paper,dict) and _key(paper.get("title"))==_key(row.get("title")) and clean(paper.get("abstract")):
        row.update(paper)
        return
    _detail(row)
    if clean(row.get("abstract")): write_json(path,{"paper":row})
def _detail(row):
    identity=re.search(r"/proceedings/(\d{4})/(\d+)/?$",clean(row.get("url")))
    if clean(row.get("pdf_url")) and identity:
        row.setdefault("identifiers",{})["doi"]=f"10.24963/ijcai.{identity.group(1)}/{int(identity.group(2))}"
        official_pdf_abstract(row,source="official_ijcai_pdf")
        return
    ...
    row["abstract"]=re.sub(r"^abstract\s*[:—-]?\s*","",text,flags=re.I)
    pdf=s.select_one('meta[name="citation_pdf_url"]'); doi=s.select_one('meta[name="citation_doi"]')
    if pdf: row["pdf_url"]=urljoin(r.url,clean(pdf.get("content")))
    if doi: row.setdefault("identifiers",{})["doi"]=clean(doi.get("content"))
    if not clean(row.get("abstract")): official_pdf_abstract(row,source="official_ijcai_pdf")
