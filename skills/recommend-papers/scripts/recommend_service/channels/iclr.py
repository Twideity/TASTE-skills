...
def fetch_metadata(spec):
    ...
    venue_id=clean(spec.get("openreview_venue_id")) or f"ICLR.cc/{year}/Conference"; rows=[]; offset=0; limit=probe_limit(spec); discovery=max(limit,10) if limit else 0; requests=[]
    while True:
        page_limit=min(1000,discovery-len(rows)) if limit else 1000
        ...
        offset+=len(notes)
        if len(notes)<page_limit or (limit and len(rows)>=discovery):break
        time.sleep(2.1)
    ...
    receipt["login_retry_errors"]=login_errors; receipt["probe_catalog_floor"]=discovery
