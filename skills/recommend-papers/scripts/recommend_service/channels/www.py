...
ID="www"; SOURCE="ACM Digital Library"
def fetch_metadata(spec): owned=dict(spec,venue_id=ID); return fetch_acm_venue(owned)
def pdf_candidates(paper:dict[str,Any]):
    rows=explicit_pdf(paper,"www_official_pdf",SOURCE); ids=paper.get("identifiers") if isinstance(paper.get("identifiers"),dict) else {}; doi=clean(ids.get("doi"))
    if doi.startswith("10.1145/"):rows.insert(0,{"url":f"https://dl.acm.org/doi/pdf/{doi}","kind":"www_acm_pdf","official_source":SOURCE})
