...
For ACM-family venues, DBLP and official accepted-paper pages establish the title pool; they are not accepted as abstract metadata. Abstract enrichment may use exact DOI/title index matches, verified OA PDF extraction, and TASTE's venue-indexed ChatPaper route. ChatPaper accepts an exact normalized title or an exact DOI for notation-only title differences, persists page/track progress, and uses the shared one-slot 10.1-second service gate. None of these fallbacks changes the single completion contract.
